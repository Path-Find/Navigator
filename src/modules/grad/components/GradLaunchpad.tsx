...
export const GradLaunchpad: React.FC = () => {
    ...
    return (
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-8 shadow-sm relative overflow-hidden">
            {/* Decorative gradient */}
            <div className="absolute top-0 right-0 w-64 h-64 bg-neutral-500/5 rounded-full blur-3xl -mr-32 -mt-32 pointer-events-none" />
            ...
            <div className="relative z-10 flex flex-col md:flex-row gap-12">
                ...
                <div className="md:w-1/3 flex flex-col gap-6">
                    <div>
                        <div className="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-neutral-50 dark:bg-neutral-900/30 text-neutral-600 mb-4 shadow-inner">
                    ...
                    <div className="bg-neutral-50 dark:bg-neutral-900/20 rounded-2xl p-5 border border-neutral-100 dark:border-neutral-800/30">
                        <div className="flex items-start gap-4">
                            <div className="mt-1 text-neutral-500">
                                <Calendar className="w-5 h-5" />
                            </div>
                            <div>
                                <h4 className="text-sm font-bold text-neutral-900 dark:text-neutral-200 mb-1">Target Intake: Fall 2026</h4>
                                <p className="text-xs font-medium text-neutral-700/70 dark:text-neutral-300">Applications typically open in Sept 2025 and close Dec-Jan.</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="space-y-4">
                        {tasks.map(task => {
                            const taskClasses = `flex items-start gap-5 p-5 rounded-2xl border-2 cursor-pointer transition-all hover:scale-[1.01] active:scale-[0.99] ${task.status === 'completed'
                                ...
                                : 'bg-white dark:bg-neutral-900 border-neutral-100 dark:border-neutral-800 hover:border-neutral-500/30 shadow-sm hover:shadow-md'
                                }`;

                            const buttonClasses = `mt-0.5 shrink-0 transition-colors ${task.status === 'completed' ? 'text-neutral-600' : 'text-neutral-300 hover:text-neutral-500'
                                }`;

                            const iconContainerClasses = `p-1.5 rounded-lg ${task.status === 'completed'
                                ? 'bg-neutral-200 dark:bg-neutral-700 text-neutral-500'
                                : 'bg-neutral-50 dark:bg-neutral-900/30 text-neutral-600'
                                }`;
