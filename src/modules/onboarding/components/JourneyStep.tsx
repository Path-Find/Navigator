                        ...
                        return (
                            <button
                                ...
                                className={`group w-full p-5 rounded-3xl border-2 text-left transition-all duration-300 relative overflow-hidden ${isSelected
                                    ? 'border-neutral-500 bg-neutral-500/5 shadow-lg shadow-neutral-500/10'
                                    : 'border-neutral-100 dark:border-neutral-800 hover:border-neutral-300 dark:hover:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-900/30'
                                    }`}
                            >
                                <div className="flex items-center gap-5 relative z-10">
                                    <div className={`w-14 h-14 rounded-2xl transition-all duration-500 flex items-center justify-center ${isSelected
                                        ? 'bg-gradient-to-br from-neutral-500 to-neutral-500 text-white shadow-lg shadow-neutral-500/30 scale-110'
                                        : 'bg-white dark:bg-neutral-800 text-neutral-400 group-hover:text-neutral-500 group-hover:scale-105 shadow-sm'
                                        }`}>
                                        {option.icon}
                                    </div>
                                    <div>
                                        <h3 className={`font-black text-lg transition-colors ${isSelected ? 'text-neutral-600 dark:text-neutral-400' : 'text-neutral-900 dark:text-white'}`}>{option.title}</h3>
