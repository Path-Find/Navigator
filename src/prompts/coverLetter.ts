export const COVER_LETTER_PROMPTS = {
  COVER_LETTER: {
    VARIANTS: {
      v1_direct: `
            ...
            INSTRUCTIONS:
            - **Grounding Rule**: Use ONLY evidence from the provided Resume Blocks. Do NOT invent skills. This is a hard constraint, not a style preference: before naming ANY specific tool, software, platform, certification, or technology, confirm it appears verbatim in the resume text. Do not add a plausible-sounding related tool to round out a list (e.g. resume says "ArcGIS Pro, ArcGIS Online" — writing "ArcGIS Pro, ArcGIS Online, and GTFS platforms" is a FAIL, because GTFS was never mentioned and the candidate cannot defend that claim in an interview). If you are not certain a named tool is in the resume, leave it out.
  ...
  CRITIQUE_COVER_LETTER: (jobDescription: string, coverLetter: string, resumeContext: string) => `
    ...
    1. TECHNICAL FIDELITY: List every specific tool, software, platform, certification, or technology named in the letter, then check each one appears verbatim in the resume. Any that don't (even a plausible-sounding one that fits the theme, e.g. adding "GTFS platforms" to a real "ArcGIS Pro, ArcGIS Online" list) is a hallucination — put it in hallucinationAlerts and this cannot score above "Weak". Also check: does it lift a resume bullet's sentence structure/phrasing with only a word or two changed? (Compare each metric-bearing sentence against its source resume bullet — same skeleton with synonyms swapped counts as a copy, even if no phrase matches exactly.)
