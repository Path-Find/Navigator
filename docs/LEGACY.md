...
These features are implemented in the codebase but are currently disabled or hidden in the UI:

- **Career Growth Module**: Includes the Coach Dashboard, Skill Tracker, Mentor/Role Model comparison, and 12-Month Roadmaps.
- **Academic / Education Module**: Includes Transcript uploading/parsing, GPA calculations, course requirement checks, and Master's Admissions (MA) eligibility evaluation.
- **Onboarding Tracks**: Pre-built onboarding paths and settings options for Career Growth (`employed`) and Education (`student`).
