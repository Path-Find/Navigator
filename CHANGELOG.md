...
- **Deterministic AI-ban detection**: `isAiBanned` is now a keyword pre-pass (`detectAiBan`) that runs before any AI call, not inside the AI extraction prompt. AI ban detection is now reliable and fast regardless of model quality. TTC and other known employers whose postings prohibit AI-assisted applications will be caught consistently.
